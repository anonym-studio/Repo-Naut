...
- [x] 🟢 `[infra]` macOS / Windows の本番ビルド成果物を Release に自動アップロード — `.github/workflows/release.yml` で macOS ユニバーサル `.dmg` を自動生成・公開
...
- [x] 🟡 `[docs]` README / AGENTS.md / 仕様書を v0.1.0 ベースに同期 — 主要機能・ディレクトリ構成・Tauri コマンド責務表を最新化、PAT/スクリプト/バックアップ/ヒートマップ/D&D 並び替えなどを反映。CHANGELOG.md / LICENSE 新規作成。LP 仕様書を統合
- [x] 🟡 `[docs]` Cloudflare Pages デプロイ手順を `docs/cloudflare-pages-deploy.md` に独立化
