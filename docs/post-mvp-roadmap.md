...
#### C-2. macOS / Windows ビルド成果物の Release 自動アップロード（規模: 中、1〜2日）

- `tauri-action` を使った GitHub Actions ワークフロー
- タグプッシュをトリガーに macOS (.dmg) / Windows (.msi) を生成して Release に添付
- 自動アップデート（A）を実装するなら、更新チェック用の JSON ファイルもここで生成する
...
5. C-2 (Release自動アップロード) — 配布準備として
6. A   (自動アップデート)       — C-2が整ってから
7. B-2 (E2Eテスト)            — C-1安定後
8. D   (ファイルログ)
9. E   (README負荷検証)
10. F-3 (ユーザーマニュアル)   — 配布直前
```
